...
router.get('/', function(req, res) {
	...
	db.Curation.count(function(err, cnt) {
		...
		if (cnt > 0) {
			db.Curation.find().toArray(function(err, curationData) {
				if (err) console.log(err);
				var pair = [];
				for ( var c in curationData) {
					pair.push(curationData[c].HGNCSymbol + ':ORPHA' + curationData[c].ORDOID);
				}
				res.render('curations_basic', { cnt:cnt, pair:pair });
			});
		}
		else res.render('curations_basic', { cnt:cnt });
	});
});
...
router.get('/:symbol/:ordoid', function(req, res) {
console.log("Here");
	var db = req.db;
	db.bind('Curation');
console.log("read in params:", req.params.symbol, req.params.ordoid);
	db.Curation.find({"HGNCSymbol":req.params.symbol, "ORDOID":req.params.ordoid}).toArray(function(err, curationData) {
		if (err) console.log(err);
console.log("Curation.Observations.CaseControlStudy.length:", curationData[0].Observations.CaseControlStudy.length);
		db.bind('Curator');
		db.Curator.find({"LogName":req.cookies.Spindle.split('-')[0]}).toArray(function(err, user) {
			if (err) console.log(err);
			db.Curation.find({"LogName":curationData[0].LogName}).toArray(function(err, creator) {
				if (err) console.log(err);
				if (curationData[0].Status == 'Final Assertion' || curationData[0].FinalAssertion) res.redirect('finalassertion', { data:curationData[0], user:user[0], creator:creator[0]});
				else if (curationData[0].Status == 'Draft Assertion' || curationData[0].DraftAssertion) res.render('draftassertion', { data:curationData[0], user:user[0], creator:creator[0]});
				else if (curationData[0].Status == 'Provisional Assertion' || curationData[0].ProvAssertion) res.render('provassertion', { data:curationData[0], user:user[0], creator:creator[0]});
				else if (curationData[0].Status == 'Summary' || curationData[0].Summary) res.render('summary', { data:curationData[0], user:user[0], creator:creator[0]});
				else if (curationData[0].Status == 'Observation' || curationData[0].Observations) res.render('observation_general', { data:curationData[0], user:user[0], creator:creator[0]});
...
*/
	db.Curation.update({"HGNCSymbol":req.body.symbol, "ORDOID":req.body.ordoid}, {$set:{"LiteratureSearch":obj, "Observations":obsv, "Status":"Observation"}}, function(err) {
		if (err) console.log(err);
