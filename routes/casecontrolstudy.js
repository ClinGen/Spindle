...
//console.log("Case group length:", casegroupstudy[0].CaseGroup.length);
							res.render('casecontrol', {data:curationData[0], user:user[0], creator:curator[0], casegroupstudy:casegroupstudy});
						});
					}
					else res.render('casecontrol', {data:curationData[0], user:user[0], creator:curator[0]});
				});
			});
	});
});
				...
				db.Curator.find({"LogName":req.cookies.Spindle.split('-')[0]}).toArray(function(err, user) {
					...
					db.Curator.find({"LogName":curationData[0].LogName}).toArray(function(err, creator) {
						...
						if (req.params.gp == 'Addnew') {
							res.render('casecontrol_group', { addnew:true, studyid:req.params.id, group:req.params.gp, studyid:req.params.id, data:curationData[0], user:user[0], creator:creator[0], casegroupstudy:casegroupstudy});
						}
						else res.render('casecontrol_group', { studyid:req.params.id, group:req.params.gp, studyid:req.params.id, data:curationData[0], user:user[0], creator:creator[0], casegroupstudy:casegroupstudy});
			...
			db.Curation.find({"HGNCSymbol": req.params.symbol, "ORDOID": req.params.ordoid}).toArray(function(err, curationData) {
				...
				db.Curator.find({"LogName":req.cookies.Spindle.split('-')[0]}).toArray(function(err, user) {
					...
					db.Curator.find({"LogName":curationData[0].LogName}).toArray(function(err, creator) {
						...
						if (casegroupstudy[n].Method) res.render('casecontrol_method', { exist:true, studyid:req.params.id, data:curationData[0], user:user[0], creator:creator[0], casegroupstudy:casegroupstudy});
						else res.render('casecontrol_method', { exist:false, studyid:req.params.id, data:curationData[0], user:user[0], creator:creator[0], casegroupstudy:casegroupstudy});
					});
				});
			});
	});
			...
			db.Curation.find({"HGNCSymbol": req.params.symbol, "ORDOID": req.params.ordoid}).toArray(function(err, curationData) {
				...
				db.Curator.find({"LogName":req.cookies.Spindle.split('-')[0]}).toArray(function(err, user) {
					...
					db.Curator.find({"LogName":curationData[0].LogName}).toArray(function(err, creator) {
						if (err) console.log(err);
						var n = parseInt(req.params.id) - 1;
						if (casegroupstudy[n].Segregation) res.render('statistic', { exist:true, studyid:req.params.id, data:curationData[0], user:user[0], creator:creator[0], casegroupstudy:casegroupstudy});
						else res.render('statistic', { exist:false, studyid:req.params.id, data:curationData[0], user:user[0], creator:creator[0], casegroupstudy:casegroupstudy});
					});
				});
			});
	});
...
//console.log("study length", casegroupstudy.length);
			...
			db.Curation.find({"HGNCSymbol": req.params.symbol, "ORDOID": req.params.ordoid}).toArray(function(err, curationData) {
				...
				db.Curator.find({"LogName":req.cookies.Spindle.split('-')[0]}).toArray(function(err, user) {
					...
					db.Curator.find({"LogName":curationData[0].LogName}).toArray(function(err, creator) {
						if (err) console.log(err);
						res.render('casegroup_group', { newstudy:true, studyid:cnt+1, group:'Addnew', data:curationData[0], user:user[0], creator:creator[0], casegroupstudy:casegroupstudy});
					});
				});
			});
		});
...
/*	var seg = '"Segregation.Curator":' + req.cookies.Spindle.split('-')[0] + ',"Segregation.DateTime":' + Date().toString();
	if (req.body.pname != '') seg += ',"Segregation.PedigreeName":' + req.body.pname;

	if (req.body.psize != '') seg += ',Segregation.PedigreeSize:' + req.body.psize;
	if (req.body.ppopulation != '') seg += ',Segregation.PedigreePopulation:' + req.body.ppopulation;
	if (req.body.pdescription != '') seg + ',Segregation.PedigreeDescription:' + req.body.pdescription;
	if (req.body.spattern != '') seg += ',Segregation.SegregationPattern:' + req.body.spattern;
	if (req.body.ppgp != '') seg += ',Segregation.PhPGenP:' + req.body.ppgp;
	if (req.body.ppgn != '') seg += ',Segregation.PhPGenN:' + req.body.ppgn;
	if (req.body.ppgu != '') seg += ',Segregation.PhPGenU:' + req.body.ppgu;
	if (req.body.pngp != '') seg += ',Segregation.PhNGenP:' + req.body.pngp;
	if (req.body.pngn != '') seg += ',Segregation.PhNGenN:' + req.body.pngn;
	if (req.body.pngu != '') seg += ',Segregation.PhNGenU:' + req.body.pngu;
	if (req.body.pugp != '') seg += ',Segregation.PhUGenP:' + req.body.pugp;
	if (req.body.pugn != '') seg += ',Segregation.PhUGenN:' + req.body.pugn;
	if (req.body.pugu != '') seg += ',Segregation.PhUGenU:' + req.body.pugu;
	if (req.body.addseginfo != '') seg += ',Segregation.AddSegInfo:' + req.body.addseginfo;

	db.CaseStudy.update({"CaseStudyID":req.body.id}, {$set:{"Segregation.Curator":req.cookies.Spindle.split('-')[0], "Segregation.DateTime":Date().toString()}}, function(err, next) {
*/
		db.CaseStudy.update({"CaseStudyID":req.body.id}, {$set:{"Segregation":seg}}, function(err, next) {
		if (err) console.log(err);
/*
		if (req.body.psize != '') {
			db.CaseStudy.update({"CaseStudyID":req.body.id}, {$set:{"Segregation.PedigreeSize":req.body.psize}}, function(err, next){
				if (err) console.log(err);
			});
		}
*/
