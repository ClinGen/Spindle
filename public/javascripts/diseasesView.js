				...
				if ($("#idValue").val() === "") {
					...
					$.get(url, function(data, status) {
						if (status === 'success') {
							...
							$("#reporttitle").html('<span> Orphanet records matched: ' + data.disData.length + '</span>')
							...
							//if (data.idType == 'FullName+Synonym') {
								...
								for (var i=0; i<data.disData.length; i++) {
									var n = i+ 1
									htmlstr += '<tr class="odd" style="height:25px"><td rowspan="5" width="30px" align="center">' + n + '</td><td style="font-weight:bold">Disease Name</td><td style="font-weight:bold;display:block">'
									...
									htmlstr += tempstr + '<div style="float:right;display:block"><a class="transfer" title="' + data.disData[i].FullName + ' -- Orphanet ID: ' + data.disData[i].ORDOID + '">^</a></div>'
									htmlstr += '</td></tr><tr class="followcontent"><td style="margin-Right:20px">Synonym(s)</td><td>'
									tempstr = ''
									...
									*/
									...
									htmlstr += '<tr class="followcontent"><td>Orphanet ID</td><td><a style="display:block" target="_blank" href="http://www.orpha.net/consor/cgi-bin/OC_Exp.php?lng=EN&Expert=' + data.disData[i].ORDOID + '">' + data.disData[i].ORDOID + '</a></td></tr>';
